...
class Notes(UserList):
    ...
    def add_note(self, note: NoteEntry):## Метод який задіяний у функціі  def make_note(name, note, teg): створеня та запису новоі нотатки.
...
@input_error
def make_note(name, note = None, teg = None): # Робить нову нотатку та додає у кінець списка. Поля не обов'язкові.
...
#     def load_note_book(self):
#         current_directory = os.getcwd()  # Отримати поточну робочу директорію
#         file_path = os.path.join(current_directory, "save_note_book.bin")
...
notes_book = Notes()
notes_book.load_note_book()  # Завантажити нотатки з файлу
